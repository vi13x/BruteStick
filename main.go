...
func ShouldSkip(currentLength int, currentCombo, target string) bool {
	if len(currentCombo) != len(target) {
		return true
	}

	for i := 0; i < len(currentCombo); i++ {
		if currentCombo[i] != target[i] {
			return false
		}
	}
	return true
...
func BruteForce(alphabet string, buffer []rune, pos, length int, startFrom string) bool {
	if pos == length {
		...
		SaveResult(password)
		SendKeys(password)
		return false // Продолжаем перебор
	}

	// Если указана начальная комбинация, пропускаем уже проверенные
...
func main() {
	...
	for length := state.CurrentLength; length <= maxLength; length++ {
		...
		// Сброс состояния для следующей длины
		state.CurrentCombo = ""
		SaveState(length+1, "")
	}
